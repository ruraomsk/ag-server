...
func makeCRC(buffer []byte, lenHeader int) (sumB int, sumP int) {
	sumP = 0
	sumB = 0
	...
	len := int(buffer[lenHeader-1])
	for i := 0; i < len; i++ {
		sumP += int(buffer[lenHeader+1+i])
		sumP = sumP & 0xffff
		sumB += int(buffer[lenHeader+1+i])
		sumB = sumB & 0xffff
	}
...
func checkCRC(buffer []byte, lenHeader int) bool {
	...
	len := int(buffer[lenHeader-1])
	tb := (int(buffer[lenHeader+len]) << 8) | int(buffer[lenHeader+len+1])
	tp := (int(buffer[lenHeader+len+2]) << 8) | int(buffer[lenHeader+len+3])
	if tb != sumB || tp != sumP {
		return false
	}
	return true
...
func takeDate(buffer []byte, pos int) time.Time {
	...
	return time.Date(year, month, day, hour, minut, sec, 0, nil)
